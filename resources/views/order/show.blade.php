    ...
    <div class="container mx-auto p-6 ">
        ...
        <div class="flex gap-10 w-full">
            <div class="flex-1 w-full">
                <div class="mb-6">
                    ...
                    @if ($order->userAddresses)
                        ...
                        <p class="text-gray-600"><strong>Số điện thoại:</strong> {{ $order->userAddresses->recipient_phone }}
                        </p>
                        <p class="text-gray-600"><strong>Địa chỉ:</strong>
                            @if ($order->userAddresses)
                                {{ $order->userAddresses->street_address }}, {{ $order->userAddresses->ward }},
                                {{ $order->userAddresses->district }},
                                {{ $order->userAddresses->city }}<strong>({{ $order->userAddresses->address_name }})</strong>
                                <br>
                            @else
                            ...
                            <li class="flex items-center space-x-4">
                                @php
                                    ...
                                    if ($detail->variant) {
                                        ...
                                        $image = $detail->product->images->firstWhere(
                                            'variant_id',
                                ...
                                @if ($detail->product->images->isNotEmpty())
                                    <a href="{{ route('products.detail', $detail->product->product_id) }}" target="_blank">
                                        <img src="{{ asset('storage/' . ($image->image_url ?? 'default.jpg')) }}"
                                            ...
                                            class="w-24 h-24 object-cover rounded-md border transition-transform duration-200 hover:scale-105 hover:shadow-md">
                                    </a>
                                @else
                                    ...
                                    @if ($detail->variant)
                                        <p class="text-sm text-gray-600 mt-1">
                                            @foreach ($detail->variant->attributes as $attribute)
                                                <span class="block">- {{ $attribute->variantAttribute->attribute_name }}:
                                                    {{ $attribute->variantAttribute->attribute_value }}</span>
                                            @endforeach
                                        </p>
                                    @endif
                                    @if ($detail->variant)
                                        <div class="flex flex-wrap gap-x-6 text-sm text-gray-600 mt-1 space-x-6">
                                            <span>Số lượng: <strong>{{ $detail->quantity }}</strong></span>
                                            <span class="text-sm text-gray-600">Giá:
                                                <strong>{{ number_format($detail->price, 0, ',', '.') }} đ</strong></span>
                                            <span class="text-sm text-gray-600">Tổng:
                                                <strong>{{ number_format($detail->total_price, 0, ',', '.') }}
                                                    đ</strong></span>
                                        </div>
                                    @endif
            ...
            <div class="flex-1 w-full">
                ...
                <div class="mb-6">
                    <p class="text-lg text-gray-800">Tạm tính: <span
                            class="font-bold text-gray-900">{{ number_format($subtotal, 0, ',', '.') }} đ</span></p>
                    <p class="text-lg text-gray-800">Phí vận chuyển: <span
                            class="font-bold text-gray-900">{{ number_format($shipping, 0, ',', '.') }} đ</span></p>

                    @if ($discount > 0)
                        <p class="text-green-600">Giảm giá đơn hàng: <span
                                class="font-bold">-{{ number_format($discount, 0, ',', '.') }} đ</span></p>
                    @endif

                    @if ($shippingDiscount > 0)
                        <p class="text-green-600">Giảm giá phí vận chuyển: <span
                                class="font-bold">-{{ number_format($shippingDiscount, 0, ',', '.') }} đ</span></p>
                    @endif
                    ...
                    @if ($isCancelled)
                        <p class="text-sm text-red-500 italic">Đơn hàng đã bị huỷ — đây là tổng tiền ban đầu trước khi huỷ.
                        </p>
                    @endif
                ...
                <div class="mt-6">
                    <a href="{{ route('order.index') }}"
                        class="bg-blue-500 text-white px-6 py-3 rounded-md hover:bg-blue-600 transition">Quay lại danh sách
                        đơn hàng</a>
                </div>
            </div>
        </div>
