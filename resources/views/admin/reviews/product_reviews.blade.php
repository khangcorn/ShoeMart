...
<div class="max-w-7xl mx-auto my-8 px-4">
    <h4 class="mb-6 text-2xl font-semibold text-gray-100">Đánh giá sản phẩm: {{ $product->name }}</h4>

    <a href="{{ route('admin.reviews.index') }}" class="inline-block mb-4 text-blue-600 hover:underline">&larr; Quay lại danh sách sản phẩm</a>
            ...
            @forelse($reviews as $review)
                <tr class="hover:bg-blue-50 transition duration-150">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center space-x-1">
                        ...
                        @if(!$review->admin_response)
                         @if(auth()->user()->hasPermission('review_reply'))
                        ...
                        @endif
                            @endif
 @if(auth()->user()->hasPermission('review_toggleHidden'))
                        <form action="{{ route('admin.reviews.toggleHidden', $review->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Bạn có chắc chắn muốn {{ $review->is_hidden ? 'hiện' : 'ẩn' }} đánh giá này?');">
                            @csrf
...
@endif
 @if(auth()->user()->hasPermission('review_destroy'))
            ...
            @empty
                <tr>
                    <td colspan="7" class="py-8 text-gray-500 italic">Chưa có đánh giá nào cho sản phẩm này.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
