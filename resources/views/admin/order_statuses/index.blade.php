...
@section('content')
<div class="px-4 py-4">
        <h4 class="text-3xl font-semibold">Trạng thái đơn hàng</h4>
         @if(auth()->user()->hasPermission('create_order_statuses'))
        <a href="{{ route('order-statuses.create') }}"   class="inline-block mt-4 duration-300 rounded-lg border border-indigo-600 bg-indigo-600 px-6 py-2 text-sm font-medium text-white focus:ring-3 focus:outline-hidden">
          Thêm trạng thái
        </a>
        @endif
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    @forelse($statuses as $status)
                        <tr class="hover:bg-gray-50">
                            <td class="border px-4 py-3 text-center items-center justify-center font-medium text-gray-600">{{ $status->status_id }}</td>
                            <td class="border px-4 py-3 text-center items-center justify-center font-medium text-gray-700">{{ $status->name }}</td>
                            <td class="border px-4 py-3 text-center items-center justify-center text-gray-700">{{ $status->description }}</td>
                            <td class="border px-4 py-3 text-center items-center justify-center">
                                <div class="flex justify-center space-x-2">
                                     @if(auth()->user()->hasPermission('edit_order_statuses'))
                                    <a href="{{ route('order-statuses.edit', $status->status_id) }}"                             class="cursor-pointer text-sm p-1.5 rounded-full bg-yellow-100 text-yellow-600 flex items-center justify-center"
                                        >
