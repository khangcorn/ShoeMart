...
<div class="px-4 py-4">
    <div class="flex items-center justify-between">
        <a class="inline-block duration-300 rounded-lg border border-indigo-600 bg-indigo-600 px-6 py-2 text-sm font-medium text-white focus:ring-3 focus:outline-hidden" href="{{ route('order-statuses.create') }}">Add New Order Status</a>
            ...
            <table class="w-full ">
                <thead >
                    <tr >
                        ...
                        <th class="px-2 py-5 border border-gray-300 text-center font-semibold">Tên trạng thái</th>
                        <th class="px-2 py-5 border border-gray-300 text-center font-semibold">Mô tả</th>
                ...
                <tbody >
                    @forelse($statuses as $status)
                        <tr >
                            <td class="border border-gray-300 dark:border-gray-700 px-2 py-5 items-center text-center">{{ $status->status_id }}</td>
                            <td class="border border-gray-300 dark:border-gray-700 px-2 py-5 items-center text-center">{{ $status->name }}</td>
                            <td class="border border-gray-300 dark:border-gray-700 px-2 py-5 items-center text-center">{{ $status->description }}</td>
                            <td class="border border-gray-300 dark:border-gray-700 px-2 py-5 items-center text-center">
                                <div class="flex justify-center items-center gap-2">
                                    <a href="{{ route('order-statuses.edit', $status->status_id) }}"
                                    ...
                                    <form action="{{ route('order-statuses.destroy', $status->status_id) }}" method="POST" 
                                        onsubmit="return confirm('Are you sure you want to delete this order status?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
