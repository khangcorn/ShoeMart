...
@section('content')
    <div class="max-w-lg mx-auto bg-white shadow-lg rounded-lg p-6 mt-10">
        ...
        <form action="{{ route('colors.update', $color->attribute_id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-gray-700 font-medium mb-2">Color Value:</label>
                <input type="text" name="attribute_value" value="{{ $color->attribute_value }}" required
