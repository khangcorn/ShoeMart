                            ...
                            @foreach (json_decode($refund->attachments) as $attachment)
                                ...
                                @elseif (in_array($ext, ['mp4', 'avi', 'mov']))
                                    <a href="{{ $url }}" target="_blank" rel="noopener" class="video-link"
                                        title="Xem video đính kèm">Xem video</a>
                                @endif
                            @endforeach
                        @else
                    ...
                    <td class="border px-4 py-2 text-center flex justify-center gap-2">
                        @if ($refund->status === 'pending')
                            @if (auth()->user()->hasPermission('process_refund'))
                                <form action="{{ route('admin.refunds.approve', $refund->refund_id) }}" method="POST"
                                    class="inline-block" onsubmit="return confirm('Bạn có chắc muốn duyệt yêu cầu này?')">
                                    @csrf
                                    <button
                                        class="bg-green-500 hover:bg-green-600 text-white font-semibold px-4 py-2 rounded">Duyệt</button>
                                </form>
                            @endif
                            @if (auth()->user()->hasPermission('reject_refund'))
                                <form action="{{ route('admin.refunds.reject', $refund->refund_id) }}" method="POST"
                                    class="inline-block" onsubmit="return confirm('Bạn có chắc muốn từ chối yêu cầu này?')">
                                    @csrf
                                    <button
                                        class="bg-red-500 hover:bg-red-600 text-white font-semibold px-4 py-2 rounded">Từ
                                        chối</button>
                                </form>
                            @endif
                        @else
