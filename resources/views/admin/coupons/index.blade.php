...
<div class="px-4 py-4">
        <h4 class="text-3xl font-semibold">Mã giảm giá</h4>
         @if(auth()->user()->hasPermission('create_coupons'))
        <a href="{{ route('coupons.create') }}"  class="inline-block duration-300 mt-4 rounded-lg border border-indigo-600 bg-indigo-600 px-6 py-2 text-sm font-medium text-white focus:ring-3 focus:outline-hidden">
            Thêm mã giảm giá
        </a>
        @endif
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    @forelse($coupons as $coupon)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 border text-gray-600">{{ $coupon->coupon_id }}</td>
                            <td class="px-4 py-3 border font-medium text-gray-700 text-lg">{{ $coupon->code }}</td>
                            <td class="px-4 py-3 border font-medium text-gray-700">{{ $coupon->apply_to }}</td>
                            <td class="px-4 py-3 border text-gray-700 capitalize">{{ $coupon->discount_type }}</td>
                            ...
                            <td class="px-4 py-3 border text-gray-700 capitalize">{{ $coupon->usage_limit }}</td>
                            <td class="px-4 py-3 border text-gray-700 capitalize">{{ $coupon->usage_count }}</td>
                            <td class="px-4 py-3 border text-gray-700 capitalize">{{ (int) $coupon->min_order_value }}đ</td>
                            <td class="px-4 py-3 border text-gray-700">
                            ...
                            <td class="px-4 py-3 border">
                                <div class="flex justify-center space-x-2">
                                    ...
                                    <a href="{{ route('coupons.edit', $coupon->coupon_id) }}"  class="cursor-pointer text-sm p-1.5 rounded-full bg-yellow-100 text-yellow-600 flex items-center justify-center">
                                    ...
                                    <form action="{{ route('coupons.destroy', $coupon->coupon_id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="cursor-pointer text-sm p-1.5 rounded-full bg-[#FEF3F2] text-[#D93948] flex items-center justify-center">
