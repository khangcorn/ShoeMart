            ...
            <table class="w-full mt-4">
                <thead class=" text-gray-700">
                    <tr class="border-b border-gray-300">
                        ...
                        <th class="border px-4 py-3">Tỉnh/Thành</th>
                        <th class="border px-4 py-3">Quận/Huyện</th>
                        <th class="border px-4 py-3">Phường/Xã</th>
                        <th class="border px-4 py-3">Phí vận chuyển</th>
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 font-medium border border-gray-300 text-center justify-center items-center text-gray-600">{{  $fee->shipping_id }}</td>
                        <td class="px-4 py-3 font-medium border border-gray-300 text-center justify-center items-center text-gray-600">{{   $fee->province }}</td>
                       
                        <td class="px-4 py-3 font-medium border border-gray-300 text-center justify-center items-center text-gray-600">{{ $fee->district ?? '-' }}</td>
                        <td class="px-4 py-3 font-medium border border-gray-300 text-center justify-center items-center text-gray-600">{{ $fee->ward ?? '-' }}</td>
                   
                        <td class="px-4 py-3 font-medium border border-gray-300 text-center justify-center items-center text-gray-600">{{ number_format($fee->fee, 0, ',', '.') }} đ</td>
                        <td class="px-4 py-3 font-medium border border-gray-300 text-center justify-center items-center text-gray-600">
                            <div class="flex justify-center space-x-2">
                                 @if(auth()->user()->hasPermission('edit_shipping_fees'))
                                <a href="{{ route('shipping-fees.edit', $fee->shipping_id) }}"   class="cursor-pointer text-sm p-1.5 rounded-full bg-yellow-100 text-yellow-600 flex items-center justify-center"
                                    >
                                ...
                                @endif
                                 @if(auth()->user()->hasPermission('delete_shipping_fees'))
